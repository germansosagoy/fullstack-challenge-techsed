...
const ProductCardDetails: React.FC<ProductCardDetailsProps> = ({ product }) => {
  ...
  return (
    <div className="flex flex-row items-center justify-center bg-white p-16 py-2 sm:py-14 sm:px-5 rounded-lg shadow w-full max-w-4xl">
      ...
      <div className="flex-shrink-0 mr-8">
       <div className="absolute top-40 z-10">
        ...
        {product.image && (
          <Image src={product.image} alt={product.title} width={300} height={300}  priority  className="object-contain object-cover rounded-lg h-80"/>
        )}
      </div>
      {/* Detalles del producto a la derecha */}
      <div className="flew-grow">
        <div className="py-4">
          <p className="text-xs text-gray-500 items-start">SKU:{" "}{product.id}</p>
        ...
        <div className="flex flex-col mt-10 space-y-4">
          ...
          {currentQuantity === 0 ? (
            ...
            <button onClick={() => removeFromCart(product.id)} className="flex justify-center items-center bg-white text-[#264b97] px-5 py-3 border-2 border-[#254a96] rounded-full text-md font-semibold transition-all hover:bg-red-600 hover:border-red-600 hover:text-white">
